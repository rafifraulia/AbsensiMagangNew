...
if (isset($_POST['tambah_mahasiswa'])) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        include '../../config/database.php';
        // Cek apakah kode pengguna sudah ada dalam tabel
        $query = mysqli_query($kon, "SELECT kode_pengguna FROM tbl_user WHERE kode_pengguna LIKE 'M%' ORDER BY kode_pengguna DESC LIMIT 1");
        if(mysqli_num_rows($query) > 0){
            $ambil= mysqli_fetch_array($query);
            $last_code = $ambil['kode_pengguna'];
            $last_number = (int)substr($last_code, 1); // Ambil angka dari kode pengguna terakhir
            $next_number = $last_number + 1;
            $kode_mahasiswa = 'M'.sprintf("%03s", $next_number);
        }else{
            $kode_mahasiswa = 'M001';
        }
        ...
        if (!empty($foto)){
            if(in_array($ekstensi, $ekstensi_diperbolehkan) === true){
                ...
                $sql="INSERT INTO tbl_mahasiswa (kode_mahasiswa,nama,universitas,jurusan,nim,mulai_magang,akhir_magang,alamat,no_telp,foto) VALUES ('$kode_mahasiswa','$nama','$universitas','$jurusan','$nim','$mulai_magang','$akhir_magang','$alamat','$no_telp','$foto')";
            }
        }else {
